...
fn new_emu_dummy<'a>(
...
) -> Unicorn<'a, ThumbTraceEmulator<'a, NullCommunication>> {
    let mut emu = <Unicorn<'a, ThumbTraceEmulator<NullCommunication>> as ThumbTraceEmulatorTrait<
        NullCommunication,
    >>::new(
        Arch::ARM,
        Mode::LITTLE_ENDIAN,
    ...
    emu.add_code_hook(
        0,
        4096,
        <Unicorn<'_, ThumbTraceEmulator<'_, NullCommunication>>>::hook_code,
    )
    .unwrap();
    emu
}
...
#[test]
fn test_hooks() {
    let leakage = NullLeakage::new();
    let mut emu = new_emu_dummy(&leakage);
...
mod tests_leakage {
    ...
    fn test_hamming_weight_leakage() {
        let leakage = HammingWeightLeakage::new();
        let mut emu = new_emu_dummy(&leakage);
        ...
        .unwrap();
        emu.emu_start(1, u64::MAX, 0, 6).unwrap();

        assert_eq!(emu.get_data().tracing.trace, vec![0.0, 1.0, 2.0, 3.0, 4.0])
    }

    #[test]
    fn test_hamming_distance_leakage() {
        let leakage = HammingDistanceLeakage::new();
        let mut emu = new_emu_dummy(&leakage);
        hook_trigger_high(&mut emu);
        emu.mem_write(
            0,
        ...
        assert_eq!(emu.get_data().tracing.trace, vec![0.0, 1.0, 1.0, 1.0, 1.0])
    }
}
